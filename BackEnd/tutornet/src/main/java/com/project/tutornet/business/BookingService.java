...
@Autowired
    ...
    public BookingResponseDto createBooking(CreateBookingDto createBookingDto) {
        ...
        Optional<Student> studentOpt = studentRepository.findById(createBookingDto.getStudentId());
        if (!studentOpt.isPresent()) {
            throw new BookingException("Student not found with ID: " + createBookingDto.getStudentId());
        }
        
        // Validate slot exists and is available
        Optional<AvailableSlot> slotOpt = availableSlotRepository.findById(createBookingDto.getSlotId());
        if (!slotOpt.isPresent()) {
            throw new BookingException("Available slot not found with ID: " + createBookingDto.getSlotId());
        }
  ...
  @Transactional
    public BookingResponseDto updateBookingStatus(UUID bookingId, UpdateBookingStatusDto updateStatusDto) {
        ...
        if (!isValidStatusTransition(oldStatus, newStatus)) {
            throw new BookingException("Invalid status transition from " + oldStatus + " to " + newStatus);
        }
    ...
    public List<Booking> getBookingsByStudentIdAndStatus(UUID studentId, String status) {
        return bookingRepository.findByStudent_UserIdAndBookingStatus(studentId, status);
    }
    ...
    private boolean isValidStatusTransition(String oldStatus, String newStatus) {
        ...
        switch (oldStatus.toUpperCase()) {
            case "PENDING":
                return newStatus.equalsIgnoreCase("CONFIRMED") || 
                       newStatus.equalsIgnoreCase("CANCELLED") ||
                       newStatus.equalsIgnoreCase("REJECTED");
            case "CONFIRMED":
                return newStatus.equalsIgnoreCase("COMPLETED") || 
                       newStatus.equalsIgnoreCase("CANCELLED") ||
    ...
    private void handleSlotStatusChange(Booking booking, String oldStatus, String newStatus) {
        AvailableSlot slot = booking.getSlot();
        
        // If booking is cancelled or rejected, make slot available
        if (newStatus.equalsIgnoreCase("CANCELLED") || newStatus.equalsIgnoreCase("REJECTED")) {
            slot.setSlotStatus("AVAILABLE");
        } 
        // If booking is confirmed from pending/rejected, mark slot as booked
        else if (newStatus.equalsIgnoreCase("CONFIRMED") && 
                (oldStatus.equalsIgnoreCase("PENDING") || oldStatus.equalsIgnoreCase("REJECTED"))) {
            slot.setSlotStatus("BOOKED");
        }
